...
double determinant(const Matrix *mat) {
  ...
  if (n == 2) {
    return mat->mtrx[0][0] * mat->mtrx[1][1] -
           mat->mtrx[0][1] * mat->mtrx[1][0];
  }

  double det = 0.0;
  // Rozwinięcie Laplace’a względem pierwszego wiersza
  for (unsigned k = 0; k < n; ++k) {
    // Tworzenie macierzy minor (pomocniczej) bez pierwszego wiersza i k-tej
    // kolumny
    ...
    for (unsigned i = 1; i < n; ++i) {
      ...
      for (unsigned j = 0; j < n; ++j) {
        if (j == k)
          continue; // Pomijamy k-tą kolumnę
    ...
    double cofactor =
        ((k % 2 == 0) ? 1.0 : -1.0) * mat->mtrx[0][k] * determinant(minor);
    det += cofactor;
  ...
  for (unsigned i = 0; i < n; ++i) {
    for (unsigned j = 0; j < n; ++j) {
      ...
      for (unsigned row = 0; row < n; ++row) {
        if (row == i)
          continue; // Pomijamy wiersz i
        minorCol = 0;
        for (unsigned col = 0; col < n; ++col) {
          if (col == j)
            continue; // Pomijamy kolumnę j
  ...
  for (unsigned i = 0; i < mat1->rows; ++i) {
    for (unsigned j = 0; j < mat2->cols; ++j) {
      ...
      for (unsigned k = 0; k < mat1->cols; ++k) {
        sum += mat1->mtrx[i][k] * mat2->mtrx[k][j];
      }
      result->mtrx[i][j] = sum;
    }
  }
